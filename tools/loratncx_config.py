...
class LoRaTNCConfig:
    ...
    def get_config(self):
        ...
        self.send_command(HW_GET_CONFIG)
        ...
        # Parse response
        # Format: CMD_SETHARDWARE, HW_GET_CONFIG, freq(4), bw(4), sf(1), cr(1), pwr(1), sync(2)
        if len(frame) < 15 or frame[0] != CMD_SETHARDWARE or frame[1] != HW_GET_CONFIG:
            return None
...
Examples:
  ...
  %(prog)s /dev/ttyUSB0 --reset
    ...
    parser.add_argument('--get-config', '-g', action='store_true', help='Get current configuration')
    ...
    has_action = (args.get_config or args.frequency or args.bandwidth or 
                  args.spreading_factor or args.coding_rate or args.power or 
                  args.syncword or args.save or args.reset)
