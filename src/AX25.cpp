...
AddrInfo parseAddresses(const uint8_t* buf, size_t len) {
  AddrInfo info;
  if (len < 14) return info; // need at least dest+src
  ...
  while (pos + 7 <= len && fields < 10) {
    ...
    fields++;
    pos += 7;
    if (last & 0x01) break; // extension bit set -> last address
  }
  if (fields < 2) return info; // invalid
  ...
  size_t hdr = fields * 7;
  if (len >= hdr + 2) hdr += 2; // control + PID
  info.header_len = hdr;
  info.ok = true;
  return info;
}
...
static void packAddressField(const String &callsign, uint8_t *out, bool last) {
  ...
  for (int i=0;i<6 && i < (int)base.length(); i++) b[i] = base[i];
  // left-shift each char by 1
  for (int i=0;i<6;i++) out[i] = ((uint8_t)b[i]) << 1;
  ...
  if (last) ssdb |= 0x01; // set extension bit to mark last address
  out[6] = ssdb;
}
...
std::vector<uint8_t> encodeUIFrame(const String &dest, const String &src, const std::vector<uint8_t> &payload) {
  ...
  for (int i=0;i<7;i++) out.push_back(a[i]);
