...
bool LoRaRadio::begin(float freq) {
  ...
  if (_paPowerPin >= 0) {
    // factory code uses ANALOG mode for the power pin
    pinMode(_paPowerPin, ANALOG);
  }
  if (_paEnPin >= 0) {
    pinMode(_paEnPin, OUTPUT);
    digitalWrite(_paEnPin, LOW); // disabled by default
  }
  if (_paTxEnPin >= 0) {
    pinMode(_paTxEnPin, OUTPUT);
    digitalWrite(_paTxEnPin, LOW); // disabled by default
  }
...
int LoRaRadio::send(const uint8_t* buf, size_t len, unsigned long timeout) {
  ...
  if (_paEnPin >= 0) digitalWrite(_paEnPin, HIGH);
  if (_paTxEnPin >= 0) digitalWrite(_paTxEnPin, HIGH);
  ...
  if (_paEnPin >= 0) digitalWrite(_paEnPin, LOW);
  if (_paTxEnPin >= 0) digitalWrite(_paTxEnPin, LOW);
